...
在本练习中，你将使用 Azure AI 代理服务和语义内核来创建一个创建费用报销电子邮件的 AI 代理。
...
1. 查看文件中的代码。 该结构包含：
    - 一些 **import** 语句，用于添加对常用命名空间的引用
    - *main* 函数，用于定义费用报销的数据（在实际应用程序中，这很可能是作为文件提交），然后调用...
    - **create_expense_claim** 函数，必须在其中添加创建和使用代理的代码
...
1. 查找注释“**使用代理生成费用报销电子邮件**”，并添加以下代码以创建代理要运行的线程，然后使用聊天消息调用它。
...
1. 查看代理的完整代码，使用注释来帮助你了解每个代码块的作用，然后保存代码更改 (**CTRL+S**)。
...
1. 应用程序完成后，查看输出。 代理应根据所提供的数据撰写费用报销的电子邮件。
